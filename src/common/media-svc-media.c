...
int _media_svc_delete_invalid_folder_items(sqlite3 *handle, const char *storage_id, const char *folder_path, bool is_recursive)
{
	...
	ret = _media_svc_get_folder_id_by_foldername(handle, storage_id, folder_path, folder_uuid);
	media_svc_retv_if(ret != MS_MEDIA_ERR_NONE, ret);
...
int _media_svc_list_query_do(sqlite3 *handle, media_svc_query_type_e query_type)
{
	...
	if (query_type == MEDIA_SVC_QUERY_SET_ITEM_VALIDITY)
		...
		ret = _media_svc_sql_query_list(handle, &g_media_svc_update_item_query_list);
	else if (query_type == MEDIA_SVC_QUERY_INSERT_FOLDER)
		ret = _media_svc_sql_query_list(handle,  _media_svc_get_folder_list_ptr());
	else
